    ...
    public List<ProcessedArticle> processArticles(List<UnprocessedArticle> unprocessedArticles)
    {
        ...
        for (UnprocessedArticle originalArticle : unprocessedArticles)
        {
            ...
            if (matches.size() > 0)
            {
                ...
                processedArticles.add(new ProcessedArticle(firstMatch.getCategoryId(), feedIds, headlines, firstMatch.getDescription(), firstMatch.getPublicationDate(), urlIds, urls));
            }
